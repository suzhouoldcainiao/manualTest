...
namespace TestDemo
{
    class StationSfrTest:StationBase
    {
        ...
        public double SFR_LU2;
        public double SFR_RU2;
        public double SFR_LD2;
        public double SFR_RD2;

        public StationSfrTest(FormTest formtest, string stationName, TextBox textbox, string type, string sn, string imageDIR,bool fiveOrnine) : base(formtest, stationName, textbox, type, sn, imageDIR)
        {
            //InitLib();
        ...
        public override bool Test()
        {
            ...
            if (GetFinalTestSFRValue(bmp, ref SFRInfo[0], win))//以后可以加入SFR数值判断
            {
                ...
                SFR_LU2 = ((ThisSFRInfo.SFR[5][0] + ThisSFRInfo.SFR[5][1]) / 2) * SFRCorner_Gain;
                SFR_RU2 = ((ThisSFRInfo.SFR[6][0] + ThisSFRInfo.SFR[6][1]) / 2) * SFRCorner_Gain;
                SFR_LD2 = ((ThisSFRInfo.SFR[7][0] + ThisSFRInfo.SFR[7][1]) / 2) * SFRCorner_Gain;
                SFR_RD2 = ((ThisSFRInfo.SFR[8][0] + ThisSFRInfo.SFR[8][1]) / 2) * SFRCorner_Gain;
                ...
                HOperatorSet.SetTposition(win, ThisSFRInfo.SearchROI[5].y, ThisSFRInfo.SearchROI[5].x);
                HOperatorSet.WriteString(win, SFR_LU.ToString("0.00"));
                HOperatorSet.SetTposition(win, ThisSFRInfo.SearchROI[6].y, ThisSFRInfo.SearchROI[6].x);
                HOperatorSet.WriteString(win, SFR_RU.ToString("0.00"));
                HOperatorSet.SetTposition(win, ThisSFRInfo.SearchROI[7].y, ThisSFRInfo.SearchROI[7].x);
                HOperatorSet.WriteString(win, SFR_LD.ToString("0.00"));
                HOperatorSet.SetTposition(win, ThisSFRInfo.SearchROI[8].y, ThisSFRInfo.SearchROI[8].x);
                HOperatorSet.WriteString(win, SFR_RD.ToString("0.00"));
        ...
        public void SaveSfrTestData(string dataPath)
        {
            string info;
            ...
            if (!File.Exists(dataPath))
            {
                ...
                info = $"SN,时间,中心,左上,右上,左下,右下";
                using (StreamWriter sw = new StreamWriter(dataPath, true))
                {
                    sw.WriteLine(info);
                }
            }
            info = $"{PSN},{DateTime.Now.ToString("HH：mm：ss")},{this.SFRCenter},{this.SFR_LU},{this.SFR_RU},{this.SFR_LD},{this.SFR_RD}";
            using (StreamWriter sw = new StreamWriter(dataPath, true))
            {
                sw.WriteLine(info);
            }
